...
#import "C:\Program Files\Common Files\System\ADO\msado15.dll"  raw_interfaces_only rename("EOF", "EndOfFile")

/////////////////////////////////////////////////////////////////////////////
struct ADOFIELD_PAIR
{
	IGRField *pGRField;
	ADODB::Field *pAdoField;
};
...
BOOL FillRecordsetFromAdoQuery(IGRRecordset *pRecordset, const CString &ConnectionString, const CString &QuerySQL)
{
	...
	try
	{
		...
		hr = pConnection.CoCreateInstance(__uuidof(ADODB::Connection));
		if ( !SUCCEEDED(hr) )
			return FALSE;
		hr = pConnection->Open(bstrConnectionString, L"", L"", ADODB::adConnectUnspecified);
		if ( !SUCCEEDED(hr) )
		{
		...
		CComPtr<ADODB::_Recordset> pAdoRecordset;
		hr = pCommand->Execute(NULL, NULL, ADODB::adCmdText, &pAdoRecordset); 
		if ( !SUCCEEDED(hr) )
		{
			pConnection->Close();
			return FALSE;
		...
		CComPtr<ADODB::Fields> pFields;    
		pAdoRecordset->get_Fields( &pFields );
		long Count;
		pFields->get_Count( &Count );
		while (Count <= 0)
		{
			CComPtr<ADODB::_Recordset> pAdoRecordset2;
